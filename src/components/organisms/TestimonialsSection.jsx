    ...
    return (
        <section className="py-20 px-4 md:px-8 max-w-7xl mx-auto bg-[#0B0B0E]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                {errorTestimony || testimony?.error && (
                    <div className="col-span-full mx-auto">
                        <ErrorCard message={testimony?.message || errorTestimony} variant="default" />
                    </div>
                )}
                {!isLoadingTestimony && !errorTestimony && testimony?.data && testimony?.data?.map((item, index) => (
